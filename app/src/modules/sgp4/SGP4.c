...
*       ----------------------------------------------------------------      */
    ...
    *    and quantities are determined. note that the result is placed in structure
    *    satrec.
    *
    *  author        : david vallado                  719-573-2600    1 mar 2001
    *
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    void twoline2rv(const char* longstr1, const char* longstr2, char typerun, char typeinput, char opsmode, int whichconst, ElsetRec* satrec)
    {
        ...
        // Epoch Year
        char epochyr_str[3];
        strncpy(epochyr_str, &longstr1[18], 2);
        epochyr_str[2] = '\0';
        satrec->epochyr = atoi(epochyr_str);

        // Epoch Days
        char epochdays_str[13];
        strncpy(epochdays_str, &longstr1[20], 12);
        epochdays_str[12] = '\0';
        ...
        char exp_str[3];
        strncpy(exp_str, &longstr1[59], 2); // Sign and digit
        exp_str[2] = '\0';
        bstar_exp = atoi(exp_str);

        satrec->bstar = bstar_mantissa * pow(10.0, bstar_exp);

        // Line 2
        // Inclination
        char inc_str[9];
        strncpy(inc_str, &longstr2[8], 8);
        inc_str[8] = '\0';
        satrec->inclo = atof(inc_str);

        // RAAN (Node)
        char node_str[9];
        strncpy(node_str, &longstr2[17], 8);
        node_str[8] = '\0';
        satrec->nodeo = atof(node_str);

        // Eccentricity
        char ecc_str[8];
        strncpy(ecc_str, &longstr2[26], 7);
        ecc_str[7] = '\0';
        satrec->ecco = atof(ecc_str) * 1.0e-7;

        // Arg Perigee
        char argp_str[9];
        strncpy(argp_str, &longstr2[34], 8);
        argp_str[8] = '\0';
        satrec->argpo = atof(argp_str);

        // Mean Anomaly
        char mo_str[9];
        strncpy(mo_str, &longstr2[43], 8);
        mo_str[8] = '\0';
        satrec->mo = atof(mo_str);

        // Mean Motion
        char no_str[12];
        strncpy(no_str, &longstr2[52], 11);
        no_str[11] = '\0';
        satrec->no_kozai = atof(no_str);
        ...
        // Calculate JD from year and epochdays
        double jd_jan1;
        double jdfrac_jan1;
        jday(year, 1, 1, 0, 0, 0.0, &jd_jan1, &jdfrac_jan1);

        satrec->jdsatepoch = jd_jan1 + (floor(satrec->epochdays) - 1.0);
        satrec->jdsatepochF = jdfrac_jan1 + (satrec->epochdays - floor(satrec->epochdays));
